...
menu_join_cb (GtkWidget *dialog, gint response, GtkEntry *entry)
{
	...
	case GTK_RESPONSE_ACCEPT:
		menu_chan_join (NULL, gtk_entry_get_text(entry));
		break;
...
static void
menu_radio_cb (GtkWidget *item, menu_entry *me)
{
	me->state = 0;
	if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(item)))
		me->state = 1;
...
menu_add_sub (GtkWidget *menu, menu_entry *me)
{
	...
	if (menu)
	{
		pos = me->pos;
		if (pos < 0)	/* position offset from end/bottom */
			pos = g_list_length (gtk_container_get_children(
                                GTK_CONTAINER(menu))) + pos;
